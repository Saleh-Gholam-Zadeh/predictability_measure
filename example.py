...
# please modify the below config including model architecture input/contex size and target/prediction length and number of epcohs ....
...
predictor_net = m2 # select any of the defined models.

transformer_learn = transformer_trainer.Learn(predictor_net, loss=cfg["learn"]["loss"], config=cfg)
...
predictor_net.load_state_dict(torch.load(cfg["learn"]["save_path"]))
...
# this inference object can be used for any model/network
transformer_infer = transformer_inference.Infer(predictor_net, config=cfg)
...
# preparing input to measure how much determinitic relations left
...
print("MI on test-set decreased from {:.5f} to {:.5f}".format(SUM_MI_initial_test,sum_res_test))
