...
def compute_normalized_mutual_information_np(freq_data_product, expfreq, basis_log_mi):
    """
    Compute normalized mutual information between freq_data_product and expfreq,
    normalized by log2(basis_log_mi), in a vectorized way.
    Parameters:
        freq_data_product (np.ndarray): A 2D NumPy array representing observed frequencies.
        expfreq (np.ndarray): A 2D NumPy array representing expected frequencies.
        basis_log_mi (int): Basis for log normalization.
    Returns:
        float: Normalized mutual information.
    """
    # Ensure inputs are NumPy arrays
    freq_data_product = np.asarray(freq_data_product)
    expfreq = np.asarray(expfreq)
    # Mask where freq_data_product is non-zero to avoid division or log issues
    mask = freq_data_product != 0
    # Compute the log term and normalize it
    log_term = np.log2(freq_data_product[mask] / expfreq[mask])
    normalized_log_term = log_term / np.log2(basis_log_mi)
    # Compute the weighted sum for mutual information
    mutual_info = np.sum(freq_data_product[mask] * normalized_log_term)
    return mutual_info
...
def get_mutual_information(data, number_output_functions=1, min_n_datapoints_a_bin=None, perm_test_flag=True, N=10):
    ...
    for i in range(0, m):
        ...
        if maxdata <= mindata:
            ...
            for point in range(0, datapoints.size):
                ...
                if datapoints[last_index:point + 1].size >= min_n_datapoints_a_bin and datapoints[point] < \
                        datapoints[
                            point + 1]:
    ...
    list_of_data_frames = []
    mutual_info = np.ones((1,
                           len(left_features) - number_output_functions + 1))  # number of featuers plus one component of the output-function
...
>>>>>>> 93fe218 (vectorized implementation of MI)
                    ...
                    mutual_info[0, j - number_output_functions + 1] = compute_normalized_mutual_information_np(freq_data_product, expfreq, basis_log_mutual_information)
